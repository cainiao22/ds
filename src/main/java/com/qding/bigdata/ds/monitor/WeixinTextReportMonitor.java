...
@Service
public class WeixinTextReportMonitor {
	private int times = 1;
	private String report = "微信文字版日报";
	private String reportTag = "WeiXinWenZiBan";
	...
	private boolean verify(String day) {
		String str = null;
		try {
			str = HttpUtil.get("https://m2.iqdnet.com/mobile/statistic?day=" + day);

			if (StringUtils.isEmpty(str) || !str.contains("北京")) {
				this.alarm(WeixinReportResult.DATA_NOT_READY, day);
				return false;
			}
			if (RegexUtil.getAllContent("<i class=\"text-orange\">([1-9]+\\.?[0-9]*)", str).size() < 10) {
