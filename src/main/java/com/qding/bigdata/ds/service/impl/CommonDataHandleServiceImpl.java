...
import com.qding.bigdata.ds.component.async.CommonDataTimeUsedHandler;
 ...
 */
@Service
public class CommonDataHandleServiceImpl extends Observable implements CommonDataHandleService, InitializingBean  {
...
//	@Autowired
	...
	public Result<List<Map<String, Object>>> execute(String jobName, Map<String, String> params) {
		...
		try {
			...
			if (params != null) {
				...
				params2.remove("track_user_name");
				params2.remove("track_user");
				params2.remove("track_source");
			}
			 ...
			 * 通知观察者
			 */
			CommonDataEvent commonDataEvent = this.getLoggingEvent(jobName, params, result);
			setChanged();
			notifyObservers(commonDataEvent);

			result.setRemark(null);
			return result;
		} catch (InvocationTargetException e) {
			if(e.getTargetException() instanceof CommonDataSearchEmptyException){
				CommonDataSearchEmptyException emptyException = (CommonDataSearchEmptyException) e.getTargetException();
				return emptyException.getResult();
				//return Result.failed(100, "任务执行失败" + e.getTargetException().getMessage());
			}
			logger.error("任务执行异常:{}", e);
			return Result.failed(90, "任务执行失败" + e.getTargetException().getMessage());
		} catch (Exception e) {
			if(e instanceof CommonDataSearchEmptyException){
				CommonDataSearchEmptyException emptyException = (CommonDataSearchEmptyException) e;
				return emptyException.getResult();
			}
			logger.error("任务执行异常:{}", e);
			return Result.failed(90, "任务执行失败" + e.getMessage());
		}
	}

	private CommonDataEvent getLoggingEvent(String jobName, Map<String, String> params, Result<List<Map<String, Object>>> result) {
		CommonDataEvent commonDataEvent = new CommonDataEvent();
		commonDataEvent.setJobName(jobName);
		commonDataEvent.setParams(params);
		commonDataEvent.setRemark(result.getRemark());
		commonDataEvent.setResult(result);
	...
	public SearchResult<CommonDataDetail> query(CommonDataDetail commonDataDetail) {
		...
		for (Map.Entry<String, List<CommonDataDetail>> entry : commonDataDetailMap.entrySet()) {
			commonDataHandlerMap.get(CommonDataHandlerEnum.getHandler(entry.getKey())).fillProperties(entry.getValue());
		}
