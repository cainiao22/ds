...
;(function($) {
    ...
    var cronInputs={
        period: '<div class="cron-select-period"><label></label><select   id="cron-period-select"  class="cron-period-select"></select></div>',
        ...
        daily: {
            tag: 'cron-daily',
            inputs: [ '<p style="display:none;"><input type="radio" name="dailyType" value="every"> Every <select class="cron-daily-select"></select> day(s)</p>',
                '']
        },
        weekly: {
            tag: 'cron-weekly',
  ...
  //  var periodOpts=arrayToOptions(["Minutes", "Hourly", "Daily", "Weekly", "Monthly", "Yearly"]);
    var periodOpts=arrayToOptions(["Daily"]);
    ...
    var eventHandlers={
        periodSelect: function() {
            ...
            $selector.siblings('div.cron-start-time').children("select.cron-clock-hour").val('2');
            switch(period) {
                case 'Minutes':
