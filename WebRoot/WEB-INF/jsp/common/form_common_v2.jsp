...
<div class="group clearfix" id="form_common">
	<label class="groupName" style="visibility: hidden;">常规条件: </label> <label
		class="span3"> 热点流: <select id="playStyle" multiple="multiple">



			<c:forEach items="${playStyleList}" var="playStyle"
				varStatus="vaStatus">

				<option value="${playStyle.id }">${playStyle.name }</option>
			</c:forEach>
	</select></label> <label class="span3">广告时长: <select id="adTime"
		multiple="multiple">



			<c:forEach items="${adTimeList}" var="adTime" varStatus="vaStatus">

				<option value="${adTime.id }">${adTime.name }</option>
			</c:forEach>
	</select></label> <label class="span3">视频类型: <select id="vtType"
		multiple="multiple">



			<c:forEach items="${vtTypeList}" var="vtType" varStatus="vaStatus">

				<option value="${vtType.id }">${vtType.name }</option>
			</c:forEach>
	</select></label>
</div>
